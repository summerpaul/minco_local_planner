/**
 * @Author: Yunkai Xia
 * @Date:   2023-09-04 15:15:54
 * @Last Modified by:   Yunkai Xia
 * @Last Modified time: 2023-09-04 15:23:38
 */
...
#include <string>
#include <vector>
namespace minco_local_planner::utils::string {
...
inline size_t Split(const std::string &src_str, const std::string sep,
...
inline size_t SplitBySpace(const std::string &src_str,
                           std::vector<std::string> &str_vec) {
  size_t start_pos = 0;
  size_t end_pos = 0;
  str_vec.clear();
...
inline std::string StripLeft(const std::string &orig_str) {
...
inline std::string StripRight(const std::string &orig_str) {
...
inline std::string Strip(const std::string &orig_str) {
...
inline std::string StripQuot(const std::string &orig_str) {
...
inline std::string RawDataToHexStr(const void *data_ptr, uint16_t data_len,
                                   bool uppercase,
                                   const std::string &delimiter) {
  if ((data_ptr == NULL) || (data_len == 0)) return std::string();
  ...
  for (uint16_t i = 0; i < data_len; ++i) {
    ...
    if (i < (data_len - 1)) oss << delimiter;
  }
  return oss.str();
}

namespace {
inline uint8_t hexCharToValue(char hex_char) {
...
inline size_t HexStrToRawData(const std::string &hex_str, void *out_ptr,
                              uint16_t out_len) {
  if ((out_ptr == NULL) || (out_len == 0)) return 0;
...
inline void _HexStrToRawDataWithoutDelimiter(const std::string &hex_str,
                                             std::vector<uint8_t> &out) {
  ...
  for (size_t i = 0; ((i * 2) < (end_pos - start_pos)); ++i) {
    ...
    uint8_t value =
        (hexCharToValue(h_char) << 4) | (hexCharToValue(l_char) & 0x0f);
    out.push_back(value);
  }
}
}  // namespace

inline size_t HexStrToRawData(const std::string &hex_str,
                              std::vector<uint8_t> &out,
                              const std::string &delimiter) {
  out.clear();
...
inline void Replace(std::string &target_str, const std::string &pattern_str,
  ...
  while (count > 0 &&
         (pos = target_str.find(pattern_str, pos)) != std::string::npos) {
