...
class Vector3 {
 public:
  ...
  inline T getLength() const { return std::sqrt(x * x + y * y + z * z); }
 ...
 *				- "Extrude to P": Create new faces by connecting P
 *with the points belonging to the horizon edge. Add the new faces to M and
...
class QuickHull {
  ...
  inline void setupInitialTetrahedron() {
    ...
    if (maxD == m_epsilonSquared) {
      ...
      auto it = std::find_if(
          m_vertexData.begin(), m_vertexData.end(), [&](const vec3 &ve) {
