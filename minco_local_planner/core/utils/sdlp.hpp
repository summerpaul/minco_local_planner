 ...
 * as long as this notice is preserved.  All standard disclaimers apply.
 *
 * Reference: Seidel, R. (1991), "Small-dimensional linear programming and
 * convex hulls made easy", Discrete & Computational Geometry 6 (1): 423–434,
...
inline int lp_no_constraints(int d, const double *n_vec, const double *d_vec,
                             double *opt) {
  int i;
...
inline void lp_min_lin_rat(int degen, const double cw_vec[2],
  ...
  if (degen) {
    ...
    /* check that the clock-wise and counter clockwise bounds are not near a
     * poles */
...
inline int wedge(const double (*halves)[2], int m, int *next, int *prev,
                 double cw_vec[2], double ccw_vec[2], int *degen) {
  int i;
  double d_cw, d_ccw;
  int offensive;
  ...
  *degen = 0;
  ...
  while (i != m) {
    ...
    if (d_ccw >= 2 * prog_epsilon) {
      ...
      return INFEASIBLE;
    } else if ((d_cw <= -2 * prog_epsilon) || (d_ccw <= -2 * prog_epsilon) ||
...
inline int linfracprog(const double *halves, /* halves  --- half spaces */
                       int istart, /* istart  --- should be zero unless doing
...
*/
{
  ...
  if (d == 1 && m != 0) {
    ...
    for (i = istart; i != m; i = next[i]) {
      /* if the optimum is not in half space i then project the problem onto
       * that plane */
      ...
      if (val < -(d + 1) * prog_epsilon) {
        ...
        /* solve sub problem */
        status = linfracprog(new_halves, 0, i, new_n_vec, new_d_vec, d - 1,
                             new_opt, new_work, next, prev, max_size);
        /* back substitution */
        if (status != INFEASIBLE) {
          ...
          double size;
          size = 0.0;
          for (j = 0; j <= d; j++) size += opt[j] * opt[j];
          size = 1.0 / sqrt(size);
          for (j = 0; j <= d; j++) opt[j] *= size;
        } else {
          return status;
        }
        /* place this offensive plane in second place */
...
inline double linprog(const Eigen::VectorXd &c, const Eigen::MatrixXd &A,
...
*/
{
  ...
  /* original allocated size is m, here changed by m + 1 for legal tail
   * accessing */
  ...
  status = sdlp::linfracprog(halves, 0, m, n_vec, d_vec, d, opt, work, next,
                             prev, m);
